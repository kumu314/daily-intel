...
def build(p: dict) -> str:
    ...
    kpi_html = (
        kpi("LPR 1Y", "cn") + kpi("联邦基金利率上限", "us")
        + kpi("核心CPI同比(7月)", "cn") + kpi("美国核心PCE(7月)", "us")
        + kpi("9月加息概率(CME)", "us")
    )
    ...
    chart_price = svg_groups(
        "物价：一边太冷，一边太热",
        [("中国（同比，越低说明东西越卖不上价）", [("CPI 同比", 0.5, "#2f7a4f"), ("核心 CPI 同比", 0.9, "#2f7a4f")], ""),
         ("美国（同比，越高说明通胀越顽固）", [("核心 CPI 同比", 2.5, "#b04a5a")], ""),
         ("美国 · 美联储最看重的口径（与中国口径不同，只作参考，不直接比）",
          [("核心 PCE 同比", 3.3, "#b04a5a"), ("核心 PCE 六个月年化", 4.1, "#8f3a26")], "口径不同")],
        note="只能跟同组比。中国 CPI 只涨 0.5% 不是好事，是东西卖不上价；美国核心 PCE 涨 3.3% 也不是好事，是通胀下不来。两边都是病，只是病的相反。")

    chart_rate = svg_groups(
        "政策利率：一个要往上加，一个动不了",
        [("中国", [("1 年期 LPR", 3.0, "#2c5f8a"), ("5 年期以上 LPR", 3.5, "#2c5f8a")], "15 个月没动"),
         ("美国", [("联邦基金利率（上限）", 3.75, "#7a4b8f")], "9 月可能加到 4.00")],
        note="中国想降，但商业银行净息差只剩 1.41%，再降就要赔本，所以降不动。美国想压通胀，市场押 9 月加息的概率在 45% 到 57% 之间。")

    chart_sector = svg_bipolar(
        "A 股本周涨跌（只放行业，指数不参与比较）",
        [("农林牧渔", 6.46, ""), ("煤炭", 5.13, ""), ("非银金融", 3.94, ""),
         ("医药生物", -1.43, ""), ("电力设备", -2.87, ""), ("通信", -3.13, "")],
        note="同周，上证指数 +1.2%，创业板指 -3.42%。指数和行业不是一类东西，所以没放进这张图——顺便说一句，这一周涨的是煤炭农林，跌的是通信和新能源，钱在往便宜的地方躲。")
...
<section><h2>中美正在往两个方向走 <small>钱变贵了，东西却卖不上价</small></h2>
...
<section><h2>A 股这一周 <small>钱在往便宜的地方躲</small></h2>
