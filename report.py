...
def build(p: dict) -> str:
    ...
    kpi_html = (
        kpi("联邦基金利率(上限)", "us") + kpi("中国CPI同比(8月)", "cn")
        + kpi("人民币兑美元(在岸)", "cn") + kpi("中国固定资产投资(1-8月)", "cn")
        + kpi("布伦特原油(9/18)", "us")
    )
    ...
    chart_sector = svg_bipolar(
        "A 股本周行业涨跌（申万一级，%）",
        [("电子", 6.1, ""), ("通信", 3.8, ""), ("房地产", 3.3, ""),
         ("农林牧渔", -2.8, ""), ("煤炭", -2.7, ""), ("石油石化", -3.7, "")],
        note="同周上证 +0.61%、创业板 +1.52%、科创 50 +6.39%、中证 1000 +3.51%；两市日均成交约 1.81 万亿，全周约 9.05 万亿。涨的是电子通信地产（科技 + 政策脉冲），跌的是石油石化煤炭农林（油价回落 + 旧能源），中小盘成长占优。")
